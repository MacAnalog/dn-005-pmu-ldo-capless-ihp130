...
class LayoutParams:
    ...
    rb_segs: int = 4           # bias resistor segments
...
BOUNDS: dict[str, tuple[float, float]] = {
    ...
    "ring_w": (0.5, 1.5), "ring_gap": (0.6, 3.0), "n_dummy": (0, 2),
...
class Builder(ObstacleMap):
    ...
    def dummy(self, name: str, w: float, l: float, pmos: bool, x0: float, y_act: float,
        ...
        # every column straight to the rail; the gate bar sits between and is crossed by them
        for xx in d.cols:
            self.m1v(xx, d.ay0 if y_rail < d.ay0 else d.ay1, y_rail)
        y_g, gb0, gb1 = d.term["_gate_bar"]  # type: ignore[misc]
        self.m1_retag(f"@g:{name}", rail)
        self.m1_claim(rail, y_g, gb0, gb1, GPAD)
        self.m1_claim_box(rail, min(d.cols) - W_M1, min(y_rail, d.ay0), max(d.cols) + W_M1,
                          max(y_rail, d.ay1))
        self.dummies.append(NR.Dummy(name, "p" if pmos else "n", rail, w, l))
        return d

    # -- guard rings -------------------------------------------------------
    def guard(self, kind: str, net: str, x0: float, y0: float, x1: float, y1: float) -> tuple[float, float, float, float]:
        """A CLOSED guard ring (``kind`` = "psub" -> ptap at vss, "nwell" -> ntap at vdd) around
        the box, its Metal1 claimed in the obstacle map so no stub can walk out through it.
        Returns the ring's outer bbox."""
        w = self.p.ring_w
        r = C.guard_ring(width=w, guardRingSpacing=0.14, guardRingType=kind,
                         bbox=((_s(x0), _s(y0)), (_s(x1), _s(y1))))
        ref = self.c << r
        b = ref.bbox()
        # the Metal1 path centre-line sits guardRingSpacing + w/2 outside the box
        d = 0.14 + w / 2
        for seg in ((x0 - d, y0 - d, x1 + d, y0 - d), (x0 - d, y1 + d, x1 + d, y1 + d)):
            self.m1_claim(net, seg[1], seg[0], seg[2], w)
        for x in (x0 - d, x1 + d):
            self.m1_claim_box(net, x - w / 2, y0 - d - w / 2, x + w / 2, y1 + d + w / 2)
    ...
    def mim(self, unit: float, x0: float, y0: float, mirror: bool = False,
        ...
        self.vstack(xb, yc, "Metal2", "Metal5", 1.3)
    ...
    def climb(self, net: str, x: float, y0: float, y1: float, w: float = 0.6) -> None:
        ...
        self.vstack(x, y0, "Metal1", "Metal2", 0.8)
...
def build_full(p: LayoutParams = LayoutParams(),
    ...
    b.guard("psub", "vss", boxA[0], boxA[1] - p.ring_gap, boxA[2], boxA[3] + p.ring_gap)
    ...
    x_isl = _s(xQ_end + p.grp_gap + 5.0)          # NW.b + both rings between the two islands
    ...
    for box in (boxQ, boxF):
        gg = p.ring_gap + 0.14 + p.ring_w + 0.6
        b.rect("NWelldrawing", box[0] - gg, box[1] - p.ring_gap - gg, box[2] + gg, box[3] + p.ring_gap + gg)
        b.guard("nwell", "vdd", box[0], box[1] - p.ring_gap, box[2], box[3] + p.ring_gap)
    ...
    yC = _s(y_out_band + p.pwr_band_w / 2 + 1.2 + GATE + GPAD / 2 + 0.6)
    ...
    y_s_pad = _s(dP.ay1 + 0.45)
    y_d_pad = _s(dP.ay0 - 0.45)
    y_s_band = _s(dP.ay1 + 0.9 + 1.2 + p.pwr_band_w / 2)
    for xx in src:
        b.rect("Metal1drawing", xx - 0.155, dP.ay1 - 0.1, xx + 0.155, dP.ay1 + 0.9)
        b.via_row("Metal1", xx, y_s_pad, p.col_vias, p.col_vias)
        b.v("Metal2drawing", xx, y_s_pad, y_s_band, 0.31)
    for xx in drn:
        b.rect("Metal1drawing", xx - 0.155, dP.ay0 - 0.9, xx + 0.155, dP.ay0 + 0.1)
        b.via_row("Metal1", xx, y_d_pad, p.col_vias, p.col_vias)
        b.v("Metal2drawing", xx, y_d_pad, y_out_band, 0.31)
    ...
    gg = p.ring_gap + 0.14 + p.ring_w + 0.6
    b.rect("NWelldrawing", boxP[0] - gg, boxP[1] - gg, boxP[2] + gg, boxP[3] + gg)
    gP = b.guard("nwell", "vdd", boxP[0], boxP[1], boxP[2], boxP[3])
    # the ntap ring's Metal1 reaches vdd through the Metal2 source spine that passes over it
    b.vstack(_s(gP[0] + p.ring_w / 2 + 0.15), _s(gP[3] - p.ring_w / 2 - 0.15), "Metal1", "Metal2", 0.5)
    # brief §7: a second, p-substrate ring between XMP and everything else, outside the combs
    b.guard("psub", "vss", gP[0] - 1.6, y_out_band - p.pwr_band_w / 2 - 1.6,
            gP[2] + 1.6, y_s_band + p.pwr_band_w / 2 + 1.6)
    ...
    for i in range(n_cols):
        ...
        if i < p.n_dummy or i >= n_cols - p.n_dummy:
            ...
            continue
        ports[order[i - p.n_dummy]].append((lo, hi))
    ...
    def _chain(entries, on_m2: bool):
        ...
        for j in range(len(entries) - 1):
            ...
            if on_m2:
                b.vstack(p0[0], p0[1], "Metal1", "Metal2", 0.38)
                b.vstack(p1[0], p1[1], "Metal1", "Metal2", 0.38)
                b.h("Metal2drawing", p0[1], p0[0], p1[0], 0.3)
            else:
                y = y_lo if at_bot else y_hi
                b.m1v(p0[0], p0[1], y, 0.3)
                b.m1v(p1[0], p1[1], y, 0.3)
                b.m1h(y, p0[0], p1[0], 0.3)
        first_free = entries[0][1]                       # link j=0 is at the bottom
        last_free = entries[-1][1] if (len(entries) - 2) % 2 == 0 else entries[-1][0]
        return first_free, last_free

    r1a, r1b = _chain(ports["XR1"], on_m2=False)
    r2a, r2b = _chain(ports["XR2"], on_m2=True)
    ...
    # brief §9: kpex extracts the Metal5 BOTTOM plate and not the top, so the terminal that HAS a
    # budget goes on top -- XCFF top = `fb` (27.9 fF), XCC bottom = `ea_out` (the lower-Z side).
    y_cc = _s(y_res - p.mim_gap - ccw - 3.0)
    bt, tp, bbcc = b.mim(ccw, _s(x_res1 + 1.4), y_cc)
    b.to_track("ea_out", *bt)
    b.to_track("ea_o1", *tp)
    y_cff = _s(bbcc[1] - p.mim_gap - cffw - 1.0)
    bt, tp, bbff = b.mim(cffw, _s(x_res1 + 1.4), y_cff)
    b.to_track("lp_brk", *bt)
    b.to_track("fb", *tp)
    ...
    for (xb, yb) in vss_pads:                     # XCOUT bottom plates -> the bottom rail
        b.vstack(xb, yb, "Metal1", "Metal2", 0.8)
        b.m1v(xb, yb, y_bot, 0.6)

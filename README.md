...
([003](experiments/003-sizing/README.md)), drawn as a schematic that provably carries the
certified cell's 25 devices and 15 nets ([004](experiments/004-schematic/README.md); its sizes
are checked too, and are still red on the four devices of M3 below) and laid out twice — the
second drawing
...
| **M3** | *(assertion landed 2026-09-05; still red on two platform gaps)* The schematic of record netlists `cap_cmim` with no `w`/`l` and `VREF vref vss 3`, and the equivalence check passes anyway. | **The claim is restated and the hole is now instrumented, but the drawing has not stopped drifting.** Topology is no longer the gap: the schematic writer's prefix-precedence bug is fixed upstream (platform #129), so the drawing carries **25 of 25** devices and 15 nets under a non-vacuous isomorphism with nothing skipped. On top of that, `build_sch.py::check_parameters` joins the certified netlist and the drawing's netlist device by device and compares **every parameter by number** (symbols resolved against the deck's own `.param` bindings, SI suffixes normalised through the platform's parsers); `w`/`l` may never be defaulted, `VREF` is checked against `vref_val` = 0.6 V by name, and the step now **exits non-zero** instead of reporting. It is red on **8 of 114** rows — `XCFF`/`XCC`/`XCOUT` drawn without `w`/`l`, `VREF` drawn as 3 V — and it reports **11** on the drawing committed at `6bd5aba`, which is the proof it bites. Both remaining causes are in the emitter and cannot be fixed from this repo: a 2-node PDK primitive shipped as a subckt (`cap_cmim`) is typed by prefix and drawn with `devices/capa.sym`, which has no size slot, though the PDK ships `sg13g2_pr/cap_cmim.sym`; and a braced `.param` expression written into a quoted xschem attribute (`value="dc {vref_val}"`) is discarded by xschem's tokenizer, which falls back to the symbol template's `value=3`. The honest claim is that the drawing **cannot drift unnoticed** — the netlist stays the design of record. [004 §2–3](experiments/004-schematic/README.md), `doc/journal/an-isomorphism-carries-no-sizes.md`. |

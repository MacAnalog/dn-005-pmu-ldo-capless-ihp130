...
(what certifies what), `doc/environment.md` (lanes and gotchas), `ldo/` module docstrings (the
...
- **Decks are built, never text-edited.** Change an `ldo.dut.Design`; let `ldo` regenerate every
  deck from it. Never edit a frozen dir — its `SHA256SUMS` is lint-pinned.
- **Batches are parallel** (`ldo.parallel.batch` / `spicexplorer_harness.batch`; the width env
...
- **Sim economy.** Expensive runs (transients, corner sets, Monte Carlo) only after the cheap
  scorecard passes the hard box (`ldo.metrics.gate` where the repo has one). Never bypass it.
...
- **Report only compact tables** (`ldo.metrics.table`) plus one paragraph of interpretation.
...
- **Write-risk** (`doc/memory/README.md`): the ledger records your runs automatically; you may
  propose journal entries with run-tag provenance in your report; you never edit `ldo/`,

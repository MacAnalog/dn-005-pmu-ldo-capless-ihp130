...
- (`005`) `Warning: singular matrix` is what ngspice prints *while* stepping to an operating point; on an extracted netlist it appears once and the analysis then converges and prints every measure. `lab.sim._FATAL` matches the substring and threw away six of thirteen benches whose results were sitting in the log. Journal: `doc/journal/singular-matrix-is-a-warning.md` (carries the proposed `lab/sim.py` diff — procedural code is human-reviewed, CLAUDE.md rule 10).

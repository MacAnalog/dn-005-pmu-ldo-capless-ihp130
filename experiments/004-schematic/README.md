...
| file | what it is |
|---|---|
| `circuits/ldo_ihp_capless/xschem/ldo_ihp_capless.sch` | **the schematic of record** (22 devices, 44 labels) — committed beside the circuit it draws, because it is the deliverable and `experiments/*/out/` is git-ignored |
...
Reproduce: `LDO_EXP=004 uv run --no-sync python experiments/004-schematic/build_sch.py`. xschem runs headless (`-n -q -r`); it needs the interpreter that has the PDK symbol libraries on `XSCHEM_LIBRARY_PATH`, which `netlist2xschem` writes into the `xschemrc` beside the `.sch`. That `xschemrc` holds absolute host PDK paths and is git-ignored — it is regenerated by every run.

...
S {}
E {}
T {tran_load_step_tb} -40 -200 0 0 0.4 0.4 {}
...
C {devices/isource.sym} 240 0 0 0 {name=ILOAD value="pulse(0.1m 10m 1u 100n 100n 10u 20u)"}
...
T {tran_load_step_tb -- load-step transient} -60 470 0 0 0.5 0.5 {}
T {drawn from decks/candidate/tran_load_step.spice; directives and .control lifted verbatim} -60 510 0 0 0.3 0.3 {}
...
C {devices/code_shown.sym} 316 570 0 0 {name=CONTROL only_toplevel=false value=".control
  set filetype=ascii
  tran 1n 8u
  meas tran vout_min  MIN  v(vout) from=1u to=8u
  meas tran vout_post FIND v(vout) AT=8u
  let v_undershoot = vout_post - vout_min
  let verr = abs(v(vout) - vout_post)
  meas tran tcross WHEN verr=1m FALL=LAST
  let t_transient = tcross - 1u
